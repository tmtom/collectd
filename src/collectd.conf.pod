...
=head2 Plugin C<humidity>

This plugin reads relativy air humidity (in %)  and temperature (in  C)  using
humidity sensor HIH6130 or HIH6131 from Honeywell.
For HW details see L<http://sensing.honeywell.com/honeywell-sensing-humidicon-hih6100-series-product-sheet-009059-6-en.pdf>.

It was developed and tested under Linux only. The only platform dependency is
the standard Linux i2c-dev interface (the particular bus driver has to
support the XXXXXXXXX  command subset).
...
=item B<Address> I<value>

Optional I2C address (0-0x7f). Default value is 0x27.
